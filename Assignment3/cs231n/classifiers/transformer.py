...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #First embed the captions
        embed = self.embedding(captions)
        #Positional encode the embeddings
        p_encod = self.positional_encoding(embed)

        #Projecting image feature, unsqueeze: 1 is inserted at shape[1]:
        projection = self.visual_projection(features).unsqueeze(1)

        #For masking the future:tril: returns lower triangle of matrix
        mask = torch.tril(torch.ones(T, T, device=p_encod.device, dtype=p_encod.dtype))

        #Transformer decoder to the caption
        features = self.transformer(tgt=p_encod,memory=projection,tgt_mask=mask)

        # Project to scores per entry.
