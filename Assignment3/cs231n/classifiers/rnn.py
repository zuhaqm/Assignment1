...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        #input 'features'*W_proj:initial hidden_state+b_proj:initial_bias
        h0 = features.dot(W_proj)+b_proj
        #x:initial_output, cache_wef = word_embedding_forward
        #(first split of caption, word vector)
        x, cache_wef = word_embedding_forward(captions_in, W_embed)
        #h:hidden_output, cache_rnn = 
        #rnn_forward(x, features input, Wx, Wh, b)
        if self.cell_type == 'rnn':
          h, cache_rnn = rnn_forward(x, h0, Wx, Wh, b)
        if self.cell_type == 'lstm':
          h, cache_rnn = lstm_forward(x, h0, Wx, Wh, b)
        #for temporal affine forward
        out, cache_taf = temporal_affine_forward(h, W_vocab, b_vocab)
        #for temporal affine backwards
        loss, dout = temporal_softmax_loss(out, captions_out, mask, verbose = False)

        #Backward pass
        
        dh, dW_vocab, db_vocab = temporal_affine_backward(dout, cache_taf)
        if self.cell_type == 'rnn':
          dx, dh0, dWx, dWh, db = rnn_backward(dh, cache_rnn)
        elif self.cell_type == 'lstm':
          dx, dh0, dWx, dWh, db = lstm_backward(dh, cache_rnn)
        dW_embed = word_embedding_backward(dx, cache_wef)
