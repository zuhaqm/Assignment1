...
def discriminator_loss(logits_real, logits_fake):
    ...
    #First finding real label
    true_labels = torch.ones(logits_real.size()).type(dtype)
    #Diff between real label and logit
    true_loss = bce_loss(logits_real, true_labels)

    #same for fake labels, .zeros as score of fake image should be zero
    fake_labels = torch.zeros(logits_fake.size()).type(dtype)
...
def generator_loss(logits_fake):
    ...
    #Only penalizing for fake image loss
    fake_labels = torch.ones(logits_fake.size()).type(dtype)
