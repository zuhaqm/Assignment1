...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j)/(norm_zi*norm_zj)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_term = torch.exp(sim(z_k, z_k_N) / tau)
        ...
        loss1 = -torch.log(exp_term / de_left)
        loss2 = -torch.log(exp_term / de_right)
...
def sim_positive_pairs(out_left, out_right):
    ...
    MM = out_left.matmul(out_right.T)  
    product = torch.diag(MM, 0).reshape(-1, 1)  
...
def compute_sim_matrix(out):
    ...
    normalized = torch.linalg.norm(out, dim=1)
    normalized_out = out / normalized.reshape(-1, 1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_btw_pairs = sim_positive_pairs(out_left, out_right)
    ...
    aug_samples = torch.cat((sim_btw_pairs, sim_btw_pairs))
    numerator = torch.exp(aug_samples / tau).to(device)
